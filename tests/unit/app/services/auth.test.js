...
import { CP_ADMIN_API, CP_AGGREGATOR_URL, LOGIN_URL } from 'app/js/constants/app_constants'
...
describe('Auth', () => {
  ...
  describe('when refreshToken is invalid', () => {
    it('call routeUtils.redirectTo with login url', (done) => {
      ...
      get('/unauthorized').then(() => {
        expect(routeUtils.redirectTo).toHaveBeenCalledWith(`${LOGIN_URL}?return_url=${CP_AGGREGATOR_URL}`)
        done()
      })
    })
  })
